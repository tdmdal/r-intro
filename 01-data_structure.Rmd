...
is.vector(a)
```
Vectors can contains any data type but it must all be of the same type. In other words, a vector cannot have mixtures of data types. There is one exception, which is with the `NA` data type. `NA` can be included within vectors of any other data type to denote missing data in that vector.
...
```
Matrix multiplication can be performed using `%*%` operator as long as the operation is mathematically possible:
```r
...
### Data frames  
Data frame are the most commonly used data structure to store data. It is a powerful two-dimensional object comprising rows and columns. The difference between data frame and matrix is that data frame can store mixture of data types. It is similar to an excel file. An easy way to understand is that data frames are basically a bunch of vectors, with their own data types and the same length, concatenated together. Below let's create a dataframe from three vectors: